...
// 👉 Instalación: registro de escuchas de eventos
function install() {
  document.addEventListener('focusin', onFocusIn, true);
  document.addEventListener('change', onPickChange, true);
  console.log('[SF Autofill] Script activo (listeners instalados).');
}

// 👇 Añadido después de install(): detección de nuevas páginas de Pre-requisito
(function monitorNewPrereqPage(){
  ...
  setInterval(() => {
    ...
    if (href !== lastUrl) {
      ...
      // Al entrar en la página “Nuevo Pre-requisito”, reiniciar estado
      if (href.includes('/lightning/o/Prerequisite__c/new' || '/lightning/r/Prerequisite__c')) {
          resetFormState();
          console.log('🔄 [Autofill] Detectado nuevo formulario de Prerequisito');
        // Pequeño retardo para permitir que LWC termine de renderizar
        setTimeout(() => { applyName(); applyComm(); }, 400);
      }
    }
  }, CHECK_INTERVAL);
})();
...
// 👇 Mantiene la lógica de instalación original
